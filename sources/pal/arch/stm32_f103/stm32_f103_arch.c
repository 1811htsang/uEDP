...
/**
 * @brief Khai báo biến kiểm tra đã khởi tạo hay chưa
 */
...
/**
 * @brief Typedef của RTC dành cho việc xử lý lỗi fatal
 */
 ...
 * @note Các khai báo này phụ thuộc vào cấu hình của STM32 và linker script được sử dụng trong dự án.
 */
...
/**
 * @brief Định nghĩa địa chỉ bắt đầu của FLASH để tính toán bộ nhớ đã sử dụng
 */
 ...
 * @note Việc bổ sung các triển khai
 *       tùy thuộc vào nhu cầu riêng của từng dự án, 
 *       có thể là khởi tạo clock, GPIO, UART, v.v.
 * @note Gọi hàm này để đảm bảo các biểu tượng linker script 
 *       được sử dụng và không bị tối ưu hóa mất
 */

void pal_stm32_init_env(void) {
  pal_memrp_get_sys_info(NULL, NULL, NULL);
  pal_stm32_nvic_config();
}

void pal_stm32_idle_sleep(void) {
	
}

void pal_stm32_nvic_config(void) {

}

void pal_stm32_exti_init(ui32 IRQnum) {
  
}

void pal_stm32_check_hardfault_reason(char* retr) {
