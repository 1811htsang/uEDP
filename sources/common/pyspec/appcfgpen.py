...
def tsm_obj_gen(kconf, target):
  ...
  for sym in kconf.unique_defined_syms:
    ...
    if sym.type == kconfiglib.STRING:
      #line content is like "extern uedp_tsm_t {relevant_name};"
      line_content = f"{indent}extern uedp_tsm_t {relevant_name};"
    else:
...
def fsm_obj_gen(kconf, target):
  ...
  for sym in kconf.unique_defined_syms:
    ...
    if "_STATE_" in relevant_name:
      continue  # Skip state symbols, only process task symbols
    if sym.type == kconfiglib.STRING:
      #line content is like "extern uedp_fsm_t {relevant_name};"
      line_content = f"{indent}extern uedp_fsm_t {relevant_name};"
    else:
      continue  # Skip unsupported types
...
def tsm_trans_gen(kconf, target):
  ...
  # Generate TSM transition content based on Kconfig configuration
  # Note: base on state name, each state result from APPCFG_FSM_TASK_{i}_STATE_{j} 
  # will be concatenated into transition table name 
  # extern const tsm_trans_t {TSM_TASK_{i}_STATE_{j}_ID}_state_trans[];
  # with TSM_TASK_{i}_STATE_{j}_ID in lowercase
