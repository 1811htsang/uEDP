 ...
 */
	...
	/** ANCHOR - Khai báo đơn vị thời gian cho timer tick
	 */
	#define UEDP_TIMER_TICK					(0x01u)

	/** ANCHOR - Định nghĩa loại Timer
	...
	/** ANCHOR - Cấu trúc một nút Timer (Timer Node)
	...
	/** ANCHOR - Khởi tạo hệ thống Timer (gọi trong uedp_init)
	...
	/** ANCHOR - Thiết lập một timer mới
	...
	/** ANCHOR - Xóa một timer dựa trên cặp Task ID và Signal
	 */
	RETR_STAT uedp_timer_remove(task_id_t tid, ui8 sig);

	/** ANCHOR - Hàm xử lý tick của timer
	 * @attention Hàm này sẽ được gọi trong ngữ cảnh ngắt
	 */
	void uedp_timer_tick(void);

	/** ANCHOR - Lấy thông tin sử dụng của Timer Pool
	...
	/** ANCHOR - Lấy giá trị tick hệ thống hiện tại
