 ...
 */
	 ...
	 */
	typedef struct uedp_msg_t {
		/* Quản lý danh sách (Linker) */
		struct uedp_msg_t* next;
	 ...
	 */
	#define uedp_msg_set_data_val(msg, val, type) \
	do { \
		if ((msg)->data != NULL) { \
			*(type*)((msg)->data) = (val); \
		} \
	 ...
	 */
	#define uedp_msg_set_data_ref(msg, ref) \
	do { \
		if ((msg)->data != NULL) { \
			*(void**)((msg)->data) = (ref); \
		} \
	 ...
	 * @return uedp_msg_t*: Con trỏ tin nhắn hoặc NULL nếu hết bộ nhớ
	 */
	 ...
	 *            giúp hệ thống UEDP có thể theo dõi và quản lý bộ nhớ một cách hiệu quả hơn.
	 *            Ví dụ khi pool_id là UEDP_MSG_TYPE_ALLOC thì info->target sẽ trỏ đến alloc_pool, info->used sẽ là số lượng tin nhắn đang được sử dụng trong alloc_pool,
