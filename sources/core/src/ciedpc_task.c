...
sta void          internal_ciedpc_task_norm_set_ready               (task_pri_t pri);
sta void          internal_ciedpc_task_norm_clear_ready             (task_pri_t pri);
sta task_pri_t    internal_ciedpc_task_norm_find_highest_priority   (void);
sta void          internal_ciedpc_task_norm_dispatch                (task_norm_t* task, ciedpc_msg_t* msg);
sta void          internal_ciedpc_task_poll_exec                    (void);
...
void ciedpc_task_poll_create(task_poll_t* task_table) {
  if (task_table) {
    ...
    // Đếm số lượng tác vụ trong bảng cho đến khi gặp ID kết thúc
    while (task_table[g_task_poll_count].id != CIEDPC_TASK_POLL_EOT_ID) {
      g_task_poll_count++;
    }
  }
}
...
RETR_STAT ciedpc_task_scheduler() {
  ...
  task_pri_t highest_pri = internal_ciedpc_task_norm_find_highest_priority();
  ...
  if (highest_pri > 0) {
    ...
    for (ui8 i = 0; i < g_task_norm_count; i++) {
      if (g_task_norm_table[i].pri == highest_pri) {
        ...
        if (msg) {
          internal_ciedpc_task_norm_dispatch(&g_task_norm_table[i], msg);
          ...
          if (fifo_is_empty(&g_task_norm_table[i].msg_queue)) {
            internal_ciedpc_task_norm_clear_ready(highest_pri);
          }
  ...
  internal_ciedpc_task_poll_exec();
...
void ciedpc_task_poll_set_ability(task_id_t tid, ui8 ability) {
  if (tid < CIEDPC_TASK_POLL_MIN_ID || tid > CIEDPC_TASK_POLL_MAX_ID) {
 ...
 */
void internal_ciedpc_task_norm_put_to_queue(task_id_t tid, ciedpc_msg_t* msg) {
 ...
 */
ciedpc_msg_t* internal_ciedpc_task_norm_get_from_queue(task_id_t tid) {
  task_norm_t* task = internal_get_task_norm_by_id(tid);
 ...
 */
void internal_ciedpc_task_norm_set_ready(task_pri_t pri) {
 ...
 */
void internal_ciedpc_task_norm_clear_ready(task_pri_t pri) {
  // Kiểm tra xem mức độ ưu tiên có hợp lệ hay không (giả sử mức độ ưu tiên hợp lệ là từ 0 đến 15)
  if (pri < CIEDPC_TASK_PRI_LEVEL_0 || pri > CIEDPC_TASK_PRI_LEVEL_15) {
    // Nếu mức độ ưu tiên không hợp lệ, có thể ghi log lỗi hoặc xử lý theo cách phù hợp
    return;
 ...
 */
task_pri_t internal_ciedpc_task_norm_find_highest_priority(void) {
 ...
 */
void internal_ciedpc_task_norm_dispatch(task_norm_t* task, ciedpc_msg_t* msg) {
 ...
 */
void internal_ciedpc_task_poll_exec(void) {
