...
void uedp_tsm_trans(uedp_tsm_t* tsm_table, tsm_state_id_t state_id) {
	...
	if (tsm_table && (state_id >= UEDP_TSM_STATE_MIN && state_id <= UEDP_TSM_STATE_MAX)) {
		...
		if (state_id == UEDP_TSM_STATE_BACK) {
			next_state = tsm_table->prev_state;
		}

		// Thực hiện exit trạng thái trước
		const tsm_state_desc_t* cur_desc = &tsm_table->state_table[tsm_table->cur_state - UEDP_TSM_STATE_MIN - UEDP_TSM_STATE_OFFSET];
    if (cur_desc->on_exit) {
...
void uedp_tsm_dispatch(uedp_tsm_t* tsm_table, uedp_msg_t* msg) {
	if (tsm_table && msg) {
		const tsm_state_desc_t* desc = &tsm_table->state_table[tsm_table->cur_state - UEDP_TSM_STATE_MIN - UEDP_TSM_STATE_OFFSET];
