 ...
 */
typedef struct uedp_msg_pool_header_t {
	uedp_msg_t* free_list;
	ui8       used_count;
	ui8       max_used;
...
sta void internal_uedp_msg_pool_init(
	...
	uedp_msg_t* pool, ui8* data_mem, 
	uedp_msg_type_t pool_type,
...
void uedp_msg_pool_init() {
	// Khởi tạo BLANK Pool
	internal_uedp_msg_pool_init(
		&g_blank_pool_ctrl, blank_pool, NULL, UEDP_MSG_TYPE_BLANK, 
		UEDP_MSG_BLANK_QUEUE_SIZE, 0, 0
	);

	// Khởi tạo ALLOC Pool
	internal_uedp_msg_pool_init(
		&g_alloc_pool_ctrl, alloc_pool, (ui8*)alloc_pool_data, UEDP_MSG_TYPE_ALLOC,
		UEDP_MSG_ALLOC_QUEUE_SIZE, UEDP_MSG_ALLOC_DATA_MAX, UEDP_MSG_ALLOC_DATA_MAX
	);
	
	// Khởi tạo EXTAL Pool
	internal_uedp_msg_pool_init(
		&g_extal_pool_ctrl, extal_pool, (ui8*)extal_pool_data, UEDP_MSG_TYPE_EXTAL,
		UEDP_MSG_EXTAL_QUEUE_SIZE, UEDP_MSG_EXTAL_DATA_MAX, UEDP_MSG_EXTAL_DATA_MAX
	);

	// Khởi tạo ISR Pool
	fifo_init(
		&isr_pool, (void*)isr_pool_buffer, 
		UEDP_MSG_ISR_QUEUE_SIZE, sizeof(uedp_msg_isr_t)
	);
}
...
uedp_msg_t* uedp_msg_alloc(ui16 des_task_id, ui16 sig, ui16 size) {
	...
	msg = internal_uedp_msg_pool_pop(pool_header);
	pal_exit_critical();
...
void uedp_msg_free(uedp_msg_t* msg) {
	...
	uedp_msg_pool_header_t* header = NULL;
...
void internal_uedp_msg_pool_init(
	uedp_msg_pool_header_t* header, uedp_msg_t* pool, ui8* data_mem, uedp_msg_type_t pool_type, ui8 units, ui16 data_size, ui16 data_max
) {
	// Kiểm tra đầu vào
	if (!header || !pool) {
		return;
	...
	for (index = 0; index < units; index++) {
		// Xóa dữ liệu rác
		memset(&pool[index], 0, sizeof(uedp_msg_t));
...
uedp_msg_t* internal_uedp_msg_pool_pop(uedp_msg_pool_header_t* header) {
	...
	uedp_msg_t* msg = header->free_list;
...
bool uedp_msg_is_valid_ptr(uedp_msg_t* msg) {
	...
	if ((msg >= &blank_pool[0] && msg < &blank_pool[UEDP_MSG_BLANK_QUEUE_SIZE]) ||
			(msg >= &alloc_pool[0] && msg < &alloc_pool[UEDP_MSG_ALLOC_QUEUE_SIZE]) ||
			(msg >= &extal_pool[0] && msg < &extal_pool[UEDP_MSG_EXTAL_QUEUE_SIZE])) {
