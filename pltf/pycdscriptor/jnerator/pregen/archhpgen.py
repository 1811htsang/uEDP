...
def main(context):
  ...
  template = env.get_template('archh.txt')
  ...
  try:
    if os.path.exists(arch_dir + f"/{context['arch_name']}/{context['arch_name']}.h"):
      print(f"[WARNING] File {arch_dir}/{context['arch_name']}/{context['arch_name']}.h already exists. Reject creation.")
      return
    else:
      print(f"[INFO] File {arch_dir}/{context['arch_name']}/{context['arch_name']}.h does not exist. Creating it.")
  ...
  with open(output_dir + "/" + context['arch_name'] + ".h", "w", encoding="utf-8") as f:
    f.write(output)