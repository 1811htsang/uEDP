# Tài liệu phân tích thiết kế kiến trúc lõi UEDP

## Kiến trúc thiết kế gốc từ AKEDP

Phiên bản AKEDP ban đầu là tiền đề cho UEDP; thiết kế gốc này đóng góp nhiều ý tưởng cốt lõi (quản lý message pool tĩnh, scheduler dựa trên bitmask, TSM/FSM nhẹ, và cơ chế xử lý ISR) nhưng cũng có một số giới hạn thực tế khi áp dụng cho các nền tảng và yêu cầu thời gian thực khắt khe hơn.
...
Hạn chế của thiết kế gốc AKEDP (tại sao cần cải tiến trong UEDP):
...
Những bài học rút ra từ hạn chế này đã trực tiếp dẫn tới các quyết định thiết kế trong UEDP (ví dụ: tách rõ hơn PAL/Core, FIFO ISR-safe bên trong core, pool sizing theo words-aligned, và giảm dependency vào heap khi có thể).

## Kiến trúc thiết kế của UEDP

UEDP được chia thành 3 tầng chức năng rõ rệt nhằm đạt mục tiêu "Zero-Touch Porting":
...
Message Manager của UEDP hiện dùng mô hình pool tĩnh kết hợp free-list để tránh phân mảnh và tránh phụ thuộc vào heap trong đường đi xử lý chính. Source hiện có 3 pool chính:
...
UEDP tách riêng đường đi từ ISR vào core bằng một FIFO nội bộ cho tín hiệu ISR. Thay vì tạo message ngay trong ngắt, ISR chỉ đăng ký cặp task ID và signal vào FIFO, còn `uedp_task_scheduler()` sẽ gọi `uedp_msg_drain_isr_pool()` ở đầu chu kỳ để rút các tín hiệu này vào luồng xử lý bình thường.
...
UEDP tách phần xuất log thành ba lớp để tránh Core phụ thuộc trực tiếp vào `printf` và đồng thời cho phép một log entry được phân phối tới nhiều backend:
...
Dựa theo thiết kế bộ nhớ quản lý tin nhắn, nhằm đảm bảo việc có thể truyền toàn bộ nội dung của một message vào payload của message khác một cách an toàn và hiệu quả, UEDP sử dụng cơ chế Data-to-Message passing. Cơ chế này cho phép người dùng truyền dữ liệu trực tiếp vào payload của message mà không cần phải lo lắng về việc quản lý bộ nhớ hoặc phân mảnh.
...
Trong UEDP, mỗi Task (Active Object) không chỉ là một hàm xử lý mà là một thực thể có "trí nhớ". Để quản lý trí nhớ này, hệ thống cung cấp hai cấp độ máy trạng thái:
...
Có thể tham khảo thiết kế chương trình mẫu trong `test01` để thấy rõ cách sử dụng TSM trong UEDP, nơi TSM được sử dụng để quản lý các chế độ vận hành của Task một cách hiệu quả và linh hoạt.
...
Có thể tham khảo thiết kế chương trình mẫu trong `test03` để thấy rõ cách sử dụng FSM trong UEDP, nơi FSM được sử dụng để quản lý logic giải mã giao thức UART một cách linh hoạt và hiệu quả.
...
UEDP khuyến khích người dùng sử dụng mô hình lồng nhau để tối ưu hóa code, trong đó:
...
UEDP sử dụng một hệ thống quản lý tín hiệu (Signal Management) để đảm bảo rằng các tín hiệu được xử lý một cách an toàn và hiệu quả trong hệ thống. Hệ thống này bao gồm:
...
Itnlog là tầng logging nội bộ của UEDP, được thiết kế để thay thế cho cách debug bằng `printf` rải rác trong luồng xử lý. Mục tiêu của itnlog là cung cấp một đường ghi log nhất quán, có thể lọc theo mức độ và tag, đồng thời dễ thay đổi đích xuất log theo từng nền tảng mà không làm Core phụ thuộc trực tiếp vào stdio.
