...
### [D2MP] Data-to-Message Passing - Cơ chế xử lý truyền dữ liệu an toàn và hiệu quả qua message
...
### [HES] Heximal Encoding Signals - Mã hóa tín hiệu theo hệ thập lục phân
...
### [PPLP] Plug-N-Play Logging Pipeline - Cơ chế logging linh hoạt và có thể mở rộng
...
`logdp` thực hiện cơ chế này thông register callback qua `pal_logdp_register()`, sau đó khi Core gọi `pal_logdp_dispatch()` với một entry, `logdp` sẽ phát entry đó tới toàn bộ callback đã đăng ký. Mỗi callback thường giữ một `pal_rprintf_service_t` riêng, copy entry vào `service->entry` rồi gọi `pal_rprintf_flush_entry()` để thực hiện việc xuất log.

`pal_logdp_dispatch()` được khuyến khích làm output function của itnlog khi dump log, giúp đảm bảo rằng cùng một entry có thể đồng thời xuất ra nhiều backend mà không phải sửa logic của Core. Ví dụ, khi dùng trên Linux, callback có thể bọc `printf` hoặc `fputs` với 1 contract đã định nghĩa để `logdp` select đích xuất phù hợp dựa trên tag của entry, ví dụ:
...
`rprintf` là nơi chứa contract để gán các hàm xuất log cụ thể cho từng backend. Ví dụ, với linux thì dùng các hàm như `printf` hoặc `fputs` rồi `fflush(stdout)`, còn với nhúng thì có thể dùng UART wrapper hoặc semihosting. Thiết kế này giúp tách biệt hoàn toàn giữa việc format log (do `xfprintf`) và việc xuất log ra đâu (do `rprintf`), đồng thời cho phép cùng một entry có thể được xuất ra nhiều backend khác nhau thông qua `logdp`.

#### Xprintf - Extended Printf

`xprintf` là tầng formatter ở mức ký tự. Nó cung cấp các API như `xprintf()`, `xfprintf()` và `xsprintf()` để format chuỗi theo cùng một bộ quy tắc trên nhiều nền tảng.
...
- **Task Polling (Application Domain):** Là một phần của logic ứng dụng nhưng không dựa trên tin nhắn. Nó vẫn nằm trong danh sách quản lý của Scheduler.
  - *Ví dụ:* Quét phím (Debounce), kiểm tra ngưỡng cảm biến.
  - *Đặc điểm:* Có "độ ưu tiên" (dù thấp hơn Norm Task) và cần chạy đều đặn.
- **Out-Context Execution (System Domain):** Là các công việc "hậu trường" phục vụ cho sự tồn tại của hệ thống.
  - *Ví dụ:* Flush itnlog, VFS Data Migration (ghi SD Card), Hardware Watchdog Feed.
  - *Đặc điểm:* Chạy khi hệ thống **hoàn toàn rảnh rỗi** (`STAT_NRDY`).

Kết luận:

- **Dùng Polling Task khi:** Cần xử lý logic ứng dụng lặp đi lặp lại và logic đó có tính chất "nhẹ", cần được đảm bảo chạy sau khi các tin nhắn được xử lý (ví dụ: LED Blink).
- **Dùng OCE khi:** Viết các dịch vụ hệ thống (System Services) mà tính chất của chúng là "dọn dẹp" dữ liệu từ Core ra ngoại vi (Logging, Storage, Cloud Sync).

#### Lợi ích của việc Tách biệt (Pros of Separation)

- **Cô lập nhiễu (Jitter Isolation):**
  - Nếu bạn đưa việc in Log (tốn thời gian) vào một Polling Task, nó sẽ làm trễ các Polling Task khác (như quét phím).
  - Khi tách ra OCE, toàn bộ logic App (Norm + Polling) đã xong xuôi. Việc Log chạy lâu hay nhanh không còn ảnh hưởng đến tính nhạy của nút nhấn nữa.
- **Quản lý tài nguyên theo "Cửa sổ cơ hội" (Idle Window):**
  - OCE chỉ thực thi khi Scheduler xác nhận: *"Tôi không còn việc gì gấp"*.
  - Thiết kế này cho phép bạn triển khai các thuật toán thông minh: *"Nếu nhịp rảnh còn 10ms, tôi mới ghi SD Card. Nếu chỉ rảnh 1ms, tôi chỉ feed Watchdog"*. Polling Task không có khả năng tự nhận biết nhịp rảnh của toàn hệ thống như vậy.
- **Tối ưu hóa năng lượng (Power Awareness):**
  - OCE là "điểm dừng chân" cuối cùng trước khi CPU đi ngủ. Nếu bạn gộp vào Polling, CPU sẽ phải thức lâu hơn để quét qua bảng Task Polling dù có thể không cần thiết. Tách ra giúp uE-OS quyết định lệnh `Sleep` chính xác hơn.
