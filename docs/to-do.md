...
- [ ] Bổ sung API filling task ID và source ID attribute của message (follow commit 6728c..) để hỗ trợ việc định danh các task và nguồn gốc của các tin nhắn trong hệ thống, giúp đảm bảo rằng các tin nhắn được xử lý một cách chính xác và hiệu quả. //NOTE - Đối với các tin nhắn được gửi không phải từ task, ví dụ như các tín hiệu khởi đầu hệ thống hoặc các tín hiệu của ISR thì sẽ có source task ID = 0xFF để chỉ ra rằng tin nhắn không đến từ một task cụ thể nào. Cần cân nhắc việc bổ sung thêm các source ID khác để phân biệt các nguồn gốc của tin nhắn trong hệ thống, ví dụ như ISR, timer, hoặc các nguồn gốc khác.
