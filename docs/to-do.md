...
- [ ] Bổ sung thiết kế vớg ustab.ankorpin và ustab.xportstax để hỗ trợ việc gắn anchor vào UST và mapping toàn bộ config vào file YAML của μE-LS, giúp giảm thiểu lỗi và tăng tính nhất quán trong việc triển khai các tính năng của lõi μEDP.
- [ ] Triển khai thiết kế ustab (Unified Symbol Table) với 2 subcomponents là cvert và gnnerate trong việc redirect kconfig data sang task-oriented YAML để hỗ trợ việc đối chiếu và quản lý các ký hiệu, hằng số và định danh trong lõi μEDP và μE-LS một cách hiệu quả và nhất quán, giúp giảm thiểu lỗi và tăng tính nhất quán trong việc triển khai các tính năng của lõi μEDP.
